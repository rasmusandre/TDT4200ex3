...
int watchEdge(unsigned int s, unsigned int yMax, unsigned int xMax, unsigned int atY,
unsigned int atX, unsigned int i, std::vector<std::vector<int>> &dwellBuffer,
				 std::complex<double> const &cmin,
				 std::complex<double> const &dc){
				 ...
				 unsigned int const blockSize)
{
	...
	for (unsigned int i = 0; i < blockSize; i++) {
		for (unsigned int s = 0; s < 4; s++){
			commonDwell = -1;
			threads[s] = std::thread(watchEdge, s, yMax, xMax, atY, atX, i, std::ref(dwellBuffer), cmin, dc);
			if (commonDwell == -1){
				return -1;
			}
		}
...
void threadWork(std::vector<std::vector<int>> &dwellBuffer,
	std::complex<double> const &cmin,
	std::complex<double> const &dc, unsigned int xstart, unsigned int xend, unsigned int ystart, unsigned int yend){
	...
	unsigned int const omitBorder = 0)
{
	...
	unsigned int num_tred = 8;
	std::thread threads[num_tred];
	unsigned int y_inc = yMax/num_tred;
	unsigned int y_m = 0;
	unsigned int y_m_last = 0;

	for (unsigned int i = 0; i < num_tred; i++){

		y_m = y_inc + y_m;

		threads[i] = std::thread(threadWork, std::ref(dwellBuffer), cmin, dc, 0, xMax, y_m_last, y_m);
		y_m_last = y_m;

	}

	for (unsigned int j = 0; j < num_tred; j++){
		threads[j].join();
	}
}
